...
2. A new JSON file is created on our host machine. This will be the policy attached to `MinimalIAMPassCreateAdd` in the next step.
...
4. Attach the policy to the user we created.
...
6. A trust policy JSON file is created for the Lambda execution role.
...
9. Using the file above create the policy for a new role which will be assigned to the Lambda.

```bash
aws iam create-policy --policy-name LambdaIAMPolicy --policy-document file://lambda_iam_perms.json
```

10. Attach the policy you created to the role.
...
2. Investigate the current roles contained within the account.

```bash
aws iam list-roles --profile PROFILE_NAME
```

3. The role which looks to be associated with Lambdas is queried. 

```bash
aws iam list-attached-role-policies --role-name LambdaExecutionRole --profile PROFILE_NAME
```

4. With the name of the policy obtained the versions are listed.

```bash
aws iam get-policy --policy-arn arn:aws:iam::ACCOUNT_NUMBER:policy/LambdaIAMPolicy --profile PROFILE_NAME
```

5. Determining the permissions authorized.
...
6. Now that we see there is high permissions present, a Lambda function is created which can attach higher permissions to the user.
...
8. At this point we have a working function but with our current permissions we can not invoke it. This is where `lambda:AddPermission` can assist us. 
...
9. **(Not Required)** We attempt to list the S3 buckets on the account, this will be denied due to our current permission set.

```bash
aws s3 ls --region PREFERRED_REGION --profile PROFILE_NAME
```

10. A new policy document is created, this will be for  the "attacker controlled" account.
...
11. The policy is added to the attacker's account.
...
12. The policy is attached to another user they are in control of.
...
13. The attacker can now invoke the function from their account.
...
14. Once invoked wait 5 mins or so for the changes to fully impact the account. Once done the new privileges will be attached.
...
6. Detach the policies from the user.
...
7. Delete the user's access key.
...
9. Delete the policy with limited permissions.
```bash
aws iam delete-policy --policy-arn arn:aws:iam::ACCOUNT_NUMBER:policy/MinimalIAMLambda
```

10. Detach the policy for external invoke from the attacker controlled account.
...
11. Delete the policy from the attacker controlled account.
